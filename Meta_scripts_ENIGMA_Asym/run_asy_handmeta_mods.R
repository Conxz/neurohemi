...
for (i in 1:length(csv_file_list)){
  ...
  if (csv_file_list[i] == 'rostralmiddlefrontal_asy_thick.csv'){
    res = rma(yi=yi, vi=vi, data = csv_dat, mods = cbind(AgeMed), control=list(stepadj=0.5)) # run the meta analysis with random effect modelling. 
  }else if (csv_file_list[i] == 'posteriorcingulate_asy_area.csv'){
    res = rma(yi=yi, vi=vi, data = csv_dat, mods = cbind(AgeMed), control=list(stepadj=0.5)) # run the meta analysis with random effect modelling. 
  }else if (csv_file_list[i] == 'supramarginal_asy_thick.csv'){
    res = rma(yi=yi, vi=vi, data = csv_dat, mods = cbind(AgeMed), control=list(stepadj=0.5)) # run the meta analysis with random effect modelling. 
  }else if (csv_file_list[i] == '_cuneus_asy_area.csv'){
    res = rma(yi=yi, vi=vi, data = csv_dat, mods = cbind(AgeMed), control=list(stepadj=0.5)) # run the meta analysis with random effect modelling. 
  }else{
...
csv_file = paste(sumout_dir, 'hand_asy_meta_mods-age.csv', sep='/') # remember to change the file name!
...
for (i in 1:length(csv_file_list)){
  ...
  if (csv_file_list[i] == 'rostralmiddlefrontal_asy_thick.csv'){
    ...
    res = rma(yi=yi, vi=vi, data = csv_dat, mods = cbind(MaleProp), control=list(stepadj=0.5)) # run the meta analysis with random effect modelling. 
  }else if (csv_file_list[i] == 'paracentral_asy_area.csv'){
    res = rma(yi=yi, vi=vi, data = csv_dat, mods = cbind(MaleProp), control=list(stepadj=0.5)) # run the meta analysis with random effect modelling. 
  }else{
...
all_res_stats = as.data.frame(all_res_stats)
colnames(all_res_stats) = c('Region', 'DOF', 'Tau2', 'Tau2SE', 'I2', 'H2', 'R2', 'Q', 'Qp','QM', 'QMp','estimate','se','zval','pval','ci.lb','ci.ub')
csv_file = paste(sumout_dir, 'hand_asy_meta_mods-sex.csv', sep='/') # remember to change the file name!
write.csv(all_res_stats, file = csv_file, row.names = FALSE)
print(paste('csv file saved for',csv_file))
...
for (i in 1:length(csv_file_list)){
  ...
  if (csv_file_list[i] == 'rostralmiddlefrontal_asy_thick.csv'){
    res = rma(yi=yi, vi=vi, data = csv_dat, mods = cbind(RightProp), control=list(stepadj=0.5)) # run the meta analysis with random effect modelling. 
  }else if (csv_file_list[i] == 'posteriorcingulate_asy_area.csv'){
    res = rma(yi=yi, vi=vi, data = csv_dat, mods = cbind(RightProp), control=list(stepadj=0.5)) # run the meta analysis with random effect modelling. 
  }else if (csv_file_list[i] == 'pericalcarine_asy_thick.csv'){
    res = rma(yi=yi, vi=vi, data = csv_dat, mods = cbind(RightProp), control=list(stepadj=0.5)) # run the meta analysis with random effect modelling. 
  }else{
...
all_res_stats = as.data.frame(all_res_stats)
colnames(all_res_stats) = c('Region', 'DOF', 'Tau2', 'Tau2SE', 'I2', 'H2', 'R2', 'Q', 'Qp','QM', 'QMp','estimate','se','zval','pval','ci.lb','ci.ub')
csv_file = paste(sumout_dir, 'hand_asy_meta_mods-hand.csv', sep='/') # remember to change the file name!
write.csv(all_res_stats, file = csv_file, row.names = FALSE)
print(paste('csv file saved for',csv_file))
...
all_res_stats = as.data.frame(all_res_stats)
colnames(all_res_stats) = c('Region', 'DOF', 'Tau2', 'Tau2SE', 'I2', 'H2', 'R2', 'Q', 'Qp','QM', 'QMp','estimate','se','zval','pval','ci.lb','ci.ub')
csv_file = paste(sumout_dir, 'hand_asy_meta_mods-icv.csv', sep='/') # remember to change the file name!
write.csv(all_res_stats, file = csv_file, row.names = FALSE)
print(paste('csv file saved for',csv_file))
