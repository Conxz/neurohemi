...
if (sys_name == 'Windows'){
  ...
  out_dir = paste0(enigma_path, '/Stats/stats/stats_asy_ageOrdered_4scanner')
  sumout_dir = paste0(enigma_path, '/Stats/stats/sum_stats_ageOrdered_4scanner')
...
csv_file = paste(sumout_dir, 'sum_asy_meta_mods-age.csv', sep='/')
...
all_res_stats = as.data.frame(all_res_stats)
colnames(all_res_stats) = c('Region', 'DOF', 'Tau2', 'Tau2SE', 'I2', 'H2', 'R2', 'Q', 'Qp','QM', 'QMp','estimate','se','zval','pval','ci.lb','ci.ub')
csv_file = paste(sumout_dir, 'sum_asy_meta_mods-sex.csv', sep='/')
write.csv(all_res_stats, file = csv_file, row.names = FALSE)
print(paste('csv file saved for',csv_file))
...
all_res_stats = as.data.frame(all_res_stats)
colnames(all_res_stats) = c('Region', 'DOF', 'Tau2', 'Tau2SE', 'I2', 'H2', 'R2', 'Q', 'Qp','QM', 'QMp','estimate','se','zval','pval','ci.lb','ci.ub')
csv_file = paste(sumout_dir, 'sum_asy_meta_mods-hand.csv', sep='/')
write.csv(all_res_stats, file = csv_file, row.names = FALSE)
print(paste('csv file saved for',csv_file))
...
all_res_stats = as.data.frame(all_res_stats)
colnames(all_res_stats) = c('Region', 'DOF', 'Tau2', 'Tau2SE', 'I2', 'H2', 'R2', 'Q', 'Qp','QM', 'QMp','estimate','se','zval','pval','ci.lb','ci.ub')
csv_file = paste(sumout_dir, 'sum_asy_meta_mods-icv.csv', sep='/')
write.csv(all_res_stats, file = csv_file, row.names = FALSE)
print(paste('csv file saved for',csv_file))


# For Scanner effects in the moderator analysis
for (i in 1:length(csv_file_list)){
  csv_file = paste(out_dir, csv_file_list[i], sep='/')
  print(csv_file)
  csv_dat = read.csv(csv_file)
  
  res = rma(yi=yi, vi=vi, data = csv_dat, mods = cbind(ScannerField)) # run the meta analysis with random effect modelling. 
  #res = rma(yi=yi, vi=vi, data = csv_dat, mods = cbind(ScannerField, ICVMed, RightProp, MaleProp))
  res_stats = c(csv_file_list[i], res$k, res$tau2, res$se.tau2, res$I2, res$H2, res$R2, res$QE, res$QEp, res$QM, res$QMp, res$b[2], res$se[2], res$zval[2], res$pval[2], res$ci.lb[2], res$ci.ub[2])
  if (i==1){
    all_res_stats = res_stats
  }else{
...
all_res_stats = as.data.frame(all_res_stats)
colnames(all_res_stats) = c('Region', 'DOF', 'Tau2', 'Tau2SE', 'I2', 'H2', 'R2', 'Q', 'Qp','QM', 'QMp','estimate','se','zval','pval','ci.lb','ci.ub')
csv_file = paste(sumout_dir, 'sum_asy_meta_mods-scanner.csv', sep='/')
write.csv(all_res_stats, file = csv_file, row.names = FALSE)
print(paste('csv file saved for',csv_file))
