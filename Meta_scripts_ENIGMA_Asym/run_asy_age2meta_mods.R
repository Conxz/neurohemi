...
out_dir = age2out_dir # remember to change!
...
for (i in 1:length(csv_file_list)){
  ...
  if (csv_file_list[i] == 'lateralorbitofrontal_asy_thick.csv'){
...
csv_file = paste(sumout_dir, 'age2_asy_meta_mods-age.csv', sep='/')# remember to change!
...
all_res_stats = as.data.frame(all_res_stats)
colnames(all_res_stats) = c('Region', 'DOF', 'Tau2', 'Tau2SE', 'I2', 'H2', 'R2', 'Q', 'Qp','QM', 'QMp','estimate','se','zval','pval','ci.lb','ci.ub')
csv_file = paste(sumout_dir, 'age2_asy_meta_mods-sex.csv', sep='/')# remember to change!
write.csv(all_res_stats, file = csv_file, row.names = FALSE)
print(paste('csv file saved for',csv_file))
...
all_res_stats = as.data.frame(all_res_stats)
colnames(all_res_stats) = c('Region', 'DOF', 'Tau2', 'Tau2SE', 'I2', 'H2', 'R2', 'Q', 'Qp','QM', 'QMp','estimate','se','zval','pval','ci.lb','ci.ub')
csv_file = paste(sumout_dir, 'age2_asy_meta_mods-hand.csv', sep='/')# remember to change!
write.csv(all_res_stats, file = csv_file, row.names = FALSE)
print(paste('csv file saved for',csv_file))
...
all_res_stats = as.data.frame(all_res_stats)
colnames(all_res_stats) = c('Region', 'DOF', 'Tau2', 'Tau2SE', 'I2', 'H2', 'R2', 'Q', 'Qp','QM', 'QMp','estimate','se','zval','pval','ci.lb','ci.ub')
csv_file = paste(sumout_dir, 'age2_asy_meta_mods-icv.csv', sep='/')  # remember to change!
write.csv(all_res_stats, file = csv_file, row.names = FALSE)
print(paste('csv file saved for',csv_file))
