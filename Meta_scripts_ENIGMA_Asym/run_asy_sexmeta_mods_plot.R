...
if (sys_name == 'Windows'){
  ...
  dataset_info_file = paste0(enigma_path, '/Stats/stats/dataset_info.csv')
  out_dir0 = paste0(enigma_path, '/Stats/stats/reuslts_adults/sexstats_asy')
  out_dir = paste0(enigma_path, '/Stats/stats/sexstats_asy_mods')
  sumout_dir = paste0(enigma_path, '/Stats//stats/sum_stats')
...
for (i in 1:length(csv_file_list)){
  ...
  plot(csv_dat$AgeMed[!is.na(csv_dat$yi)], csv_dat$yi[!is.na(csv_dat$yi)], 
       pch=19, cex=size, xlab="Meidan Age (year)", main = str_replace(csv_file_list[i], '.csv', ''),
       ylab="Sex Effect in Asymmetry (Male - Female)", las=1, bty="l")
