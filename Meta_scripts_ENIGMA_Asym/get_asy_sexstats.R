...
for (col_i in 1:length(col_names_list)){
  ...
  for (i in 1:length(dataset_list)){
    ...
    tmp.se = as.data.frame(t(se.main))
    colnames(tmp.se) = colnames(d.main)
    
    tmp.n1 = as.data.frame(t(n.grp1.main))
    colnames(tmp.n1) = colnames(d.main)
    
    tmp.n2 = as.data.frame(t(n.grp2.main))
    colnames(tmp.n2) = colnames(d.main)
    ...
    if (!is.na(col_name) && !is.na(tmp.n2[[col_name]])){ # to exclude stats without or with less than 15 subjects in one group
      eff_n1 = tmp.n1[[col_name]]
      eff_n2 = tmp.n2[[col_name]]
      if (eff_n1>=N_grp_threshold && eff_n2>=N_grp_threshold){
        eff_d = tmp[[col_name]]
        eff_dav = tmp.se[[col_name]]^2
      }else{
        eff_d = NA
        eff_dav = NA
      }
  ...
  write.csv(dataset_info, file = paste(sexout_dir, csv_file_name, sep='/'), row.names = FALSE)
...
for (j in 1:length(area_list)){
  ...
  for (i in 1:length(dataset_list)){
    ...
    if (!is.na(tmp_col_name) && !is.na(tmp.n2[[tmp_col_name]])){ # to exclude stats without or with less than 15 subjects in one group
      eff_n1 = tmp.n1[[tmp_col_name]]
      eff_n2 = tmp.n2[[tmp_col_name]]
      if (eff_n1>=N_grp_threshold && eff_n2>=N_grp_threshold){
        eff_d = tmp[[tmp_col_name]]
        eff_dav = tmp.se[[tmp_col_name]]^2
      }else{
        eff_d = NA
        eff_dav = NA
      }
...
for (j in 1:length(area_list)){
  ...
  csv_file_name = paste(area_str, '_asy_area.csv', sep='') # change thick to area
  write.csv(dataset_info, file = paste(sexout_dir, csv_file_name, sep='/'), row.names = FALSE)
  print(paste('csv file saved for',csv_file_name))
}
