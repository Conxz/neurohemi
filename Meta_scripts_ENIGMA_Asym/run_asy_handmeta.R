...
for (i in 1:length(csv_file_list)){
  ...
  if (csv_file_list[i] == 'rostralmiddlefrontal_asy_thick.csv'){
    res = rma(yi=yi, vi=vi, data = csv_dat, control=list(stepadj=0.5)) # run the meta analysis with random effect modelling. 
...
csv_file = paste(sumout_dir, 'hand_asy_meta.csv', sep='/')  # remember to change the file name!
