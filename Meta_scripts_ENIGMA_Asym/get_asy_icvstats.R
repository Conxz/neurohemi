...
for (col_i in 1:length(col_names_list)){
  ...
  for (i in 1:length(dataset_list)){
    ...
    if (file.exists(eff_file)){
      ...
      if (!is.na(col_name) ){ # to exclude stats without data or age range less than 5 years
        eff_r = tmp[[col_name]][3]
        eff_rav = tmp.se[[col_name]][3]^2
      }else{
        eff_r = NA
        eff_rav = NA
      }
...
for (j in 1:length(area_list)){
  ...
  for (i in 1:length(dataset_list)){
    ...
    if (file.exists(eff_file)){
      ...
      if (!is.na(tmp_col_name) ){ # to exclude stats without data or age range less than 5 years
        eff_r = tmp[[tmp_col_name]][3]
        eff_rav = tmp.se[[tmp_col_name]][3]^2
      }else{
        eff_r = NA
        eff_rav = NA
      }
